<?php
	...
	class FPBatis {
		 ...
		 */
		function doSelectS($id, $params=null, $debug=false) {
			$ids = split("\.", $id);
		 ...
		 */
		function doSave($namespace, $obj, $key='room_id', $insertId='insert', $updateId='update', $newValue=-1) {
		...
		function doSaveForm($namespace, $key='room_id', $insertId='insert', $updateId='update', $newValue=-1) {
		...
		function doSaveFormm($namespace, $key='flag', $insertId='insert', $updateId='update', $newValue=-1) {
			// 회원 처리에 있어서 flag는 insert인지 update인지 구분해 주는 변수이다.
			if ($this->param($key) == -1)
				return $this->doInsert($namespace . '.' . $insertId,array(),true);
			else
				return $this->doUpdate($namespace . '.' . $updateId,array(),true);
